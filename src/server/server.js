...
const mainSavePath = GetResourcePath(resName) + '/html/assets/icon/items';
...
try {
	...
	onNet('takeScreenshot', async (name, pedType, drawable) => {
		const savePath = `${mainSavePath}/${name}/${pedType}`;
		...
		exports['screenshot-basic'].requestClientScreenshot(
			source,
			{
				fileName: `${savePath}/${drawable}_.png`,
				encoding: 'png',
				quality: 1.0,
			},
			async (err, fileName) => {
